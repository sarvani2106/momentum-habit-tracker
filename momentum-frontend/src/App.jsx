...
import Hero3D from './components/Hero3D'; // <-- Swapped to your new 3D component!
...
function App() {
  ...
  // --- UPDATED: Normal, grounded motivational messages ---
  const handleCheckOff = (habitId, habitName) => { 
    ...
    .then(() => { 
      ...
      const motivations = [
        `Great job! ${habitName} is done.`,
        `Checkmark earned: ${habitName} complete.`,
        `One step closer. Excellent work on ${habitName}!`,
        `Consistency wins. You finished ${habitName}!`
      ];
    ...
    <div className={theme === 'dark' ? 'dark' : ''}>
      <div className="min-h-screen bg-[#fafafa] dark:bg-[#050505] text-neutral-900 dark:text-neutral-100 transition-colors duration-500 font-sans selection:bg-neutral-300 dark:selection:bg-neutral-800 relative pt-24 pb-12">
        ...
        <main className="max-w-4xl mx-auto px-6 relative z-10">
          {!loggedInUser ? (
            <div className="space-y-12 mt-20 max-w-3xl mx-auto">
            ...
            <div className="space-y-8">
              ...
              {/* --- UPDATED: The 3D Hero Component --- */}
              <Hero3D username={loggedInUser.username} />
              ...
              <div className="bg-white/60 dark:bg-white/[0.02] backdrop-blur-2xl p-8 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none">
                <form onSubmit={handleAddHabit} className="flex flex-col sm:flex-row gap-3">
                  ...
                  {/* --- UPDATED: Button says 'Add Task' --- */}
                  <button type="submit" className="bg-neutral-100 dark:bg-white/10 hover:bg-neutral-200 dark:hover:bg-white/20 text-neutral-900 dark:text-white font-semibold px-6 py-3 rounded-full transition-colors cursor-pointer text-sm">Add Task</button>
                </form>
              </div>
              ...
              <div className="bg-white/60 dark:bg-white/[0.02] backdrop-blur-2xl p-8 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none">
                ...
                {habits.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {habits.map(habit => (
                      <div key={habit.id} className="flex justify-between items-center bg-white dark:bg-white/5 p-4 pl-6 rounded-2xl border border-black/5 dark:border-white/5 hover:border-blue-500/30 transition-colors">
                        <div>
                          ...
                          <div className="flex gap-4 mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                            <span className={habit.currentStreak >= 3 ? "text-blue-500" : ""}>🔥 {habit.currentStreak || 0} Streak</span>
                            <span className="text-amber-500/80">🏆 {habit.longestStreak || 0} Best</span>
                          </div>
                        </div>
